...
from temp_models import GeneratorUNet, Discriminator
...
parser.add_argument("--channels", type=int, default=7, help="number of image channels")
...
# ここからデータセット作成
from datasets1 import ImageDataset

# Pytorchのデータセット作成関数
dataloader = DataLoader(
    ImageDataset("../MATLAB/SCGAN/ISTD_Dataset/train" , hr_shape=hr_shape),
...
for epoch in range(opt.epoch, opt.n_epochs): #エポック
    for i, imgs in enumerate(dataloader):    #エポック内でどの画像を使うか
        ...
        # 学習に使う画像の取り出し
        RF = torch.cat([imgs["shadow"],imgs["mask"]], 1).to(device) #入力
        B = imgs["removal"].to(device)   #正解

        # discriminator用の正解ラベル
        valid = torch.randn((RF.size(0), *discriminator.output_shape), device = device, requires_grad = False) * 0.1 + 0.5 #本物
        fake = torch.randn((RF.size(0), *discriminator.output_shape), device = device, requires_grad = False) * 0.1 - 0.5  #偽物
        ...
        # Adversarial loss
        loss_GAN = criterion_GAN(discriminator(torch.cat([RF, B_hat], 1 )),valid)
        ...
        # ロスを計算 detatchが重要
        loss_real = criterion_GAN(discriminator(torch.cat([RF, B], 1 )), valid)
        loss_fake = criterion_GAN(discriminator(torch.cat([RF, B_hat], 1 ).detach()), fake)
        ...
        if batches_done % opt.sample_interval == 0:
            img_grid = gridImage((RF[:,:3,:,:], B_hat, B))
