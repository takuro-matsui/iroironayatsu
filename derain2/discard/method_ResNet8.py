...
for epoch in range(opt.epoch, opt.n_epochs): #エポック
    for i, imgs in enumerate(dataloader):    #エポック内でどの画像を使うか
        ...
        ground_truth = imgs["ground_truth"].to(device)   #正解
        ...
        valid = torch.randn((rainy_image.size(0), *discriminator.output_shape), device = device, requires_grad = False) * 0.001 + 0.5 #本物
        fake = torch.randn((rainy_image.size(0), *discriminator.output_shape), device = device, requires_grad = False) * 0.001 - 0.5  #偽物
