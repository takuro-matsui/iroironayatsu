...
parser.add_argument("--d_channels", type=int, default=6, help="number of image channels")
...
# Discriminator
parser.add_argument("--adv_rate", type=float, default=0.001, help="ratio of adversarial loss for loss_G")
parser.add_argument("--d_label", type=str, default=0, help="0: +-0.5+noise, 1: +-0.5, 2: 0/1")
...
discriminator = Discriminator(input_shape=(opt.d_channels, *hr_shape))
...
for epoch in range(opt.epoch, opt.n_epochs): #エポック
    for i, imgs in enumerate(dataloader):    #エポック内でどの画像を使うか
        ...
        if opt.d_label == 0:
            ...
            valid = torch.ones((rainy_image.size(0), *discriminator.output_shape), device = device, requires_grad = False) * 0.5 #本物
            fake = torch.ones((rainy_image.size(0), *discriminator.output_shape), device = device, requires_grad = False) * (-0.5)  #偽物
        elif opt.d_label == 2:
            valid = torch.ones((rainy_image.size(0), *discriminator.output_shape), device = device, requires_grad = False) #本物
            fake = torch.zeros((rainy_image.size(0), *discriminator.output_shape), device = device, requires_grad = False)
        ...
        elif opt.d_channels == 3:
            loss_GAN = criterion_GAN(discriminator(B_hat), valid)


        # ロスを合計する 重み付けが大事
        if opt.adv_rate > 0:
            loss_G = opt.adv_rate * loss_GAN + loss_l2
        else: 
            loss_G = loss_l2
        ...
        # ロスを計算 detatchが重要
        if opt.d_channels == 6:
            loss_real = criterion_GAN(discriminator(torch.cat([rainy_image, B], 1 )), valid)
            loss_fake = criterion_GAN(discriminator(torch.cat([rainy_image, B_hat], 1 ).detach()), fake)
        elif if opt.d_channels == 3:
            loss_real = criterion_GAN(discriminator( B), valid)
            loss_fake = criterion_GAN(discriminator(B_hat.detach()), fake)
        # Total loss
