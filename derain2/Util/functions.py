...
def RGB2YCBCR(img):
    ...
    #cb = 0.595*img[:,:,0]-0.274*img[:,:,1]+0.322*img[:,:,2]
    #cr = 0.211*img[:,:,0]-0.523*img[:,:,1]+0.312*img[:,:,2]
...
def OutputRainNoise(image, rain_number):
    if rain_number == 0:
        # parameters
        #motion_length = 15 + (np.random.randint(6) - np.random.randint(7) + 1) # length (default: 15)
        ...
        #uniform_noise_amount = 1.2 + 0.1 * (np.random.rand() - np.random.rand()) #rain noise amount (default: 1.2)
...
def ConfidenceMap(image, rain_noise, rainy_image):
    residual = rainy_image - image
    eps = 0.00001
    c_map = image
    for i in range(3):
        c_map[:,:,i] = residual[:,:,i] / (rain_noise+eps)
    return c_map
